...
cricbuzz = Cricbuzz()
import streamlit as st
st.set_page_config(page_title="News Page", page_icon="📰")
...
# Function to fetch series information
def get_series_info():
    cricbuzz = Cricbuzz()
    cric_series = cricbuzz.get_series(type="international")
    return cric_series
...
# Streamlit app
# Function to fetch series from archive
def get_series_from_archive():
    cricbuzz = Cricbuzz()
    cric_series = cricbuzz.get_series_from_archive(year=2023, type="international")
    return cric_series

# Function to fetch matches by day
...
def get_team_schedule(team_id):
    cricbuzz = Cricbuzz()
    team_schedule = cricbuzz.get_team_schedule(team_id)
    return team_schedule
def display_upcoming_matches(upcoming_matches):
    #st.header("Upcoming Matches")
    if upcoming_matches:
        match_data = []
        headings = ["Series Name", "Match Name", "Start Date & Time", "Location"]

        # Append headings to match_data
        match_data.append(headings)
        ...
        st.markdown("<h3 style='color: red;'>No Upcoming Matches</h3>", unsafe_allow_html=True)

# Function to fetch team results
def get_team_results(team_id):
    cricbuzz = Cricbuzz()
    team_results = cricbuzz.get_team_results(team_id)
    return team_results

# Streamlit app
def main():
    st.markdown("<h1 style='text-align: center;'>News</h1>", unsafe_allow_html=True)
    ...
    if option == "Live Matches":
        st.markdown("<h2 style='text-align: left;'>Live Matches</h2>", unsafe_allow_html=True)
        live_matches = get_live_matches()
        print(live_matches)        
        # Check if there are live matches
        if live_matches != [{'error': 'No live matches'}]:
            for match in live_matches:
                print(match["match_name"], end="\t")
                print(match["status"])
        else:
            st.markdown("<h3 style='color: red;'>No Live Matches</h3>", unsafe_allow_html=True)
    # Display data based on the selected option

    if option == "Upcoming Matches":
        st.header("Upcoming Matches")
        upcoming_matches = get_upcoming_matches()
        display_upcoming_matches(upcoming_matches)

        # for match in upcoming_matches:
        #     st.write(match["match_name"], end="\t")
        #     st.write(match["status"])

    elif option == "Series Information":
        st.header("Series Information")
        cric_series = get_series_info()
        
        for series in cric_series:
            st.write(series)
            for matches in cric_series[series]:
                st.write("\t", matches["series_name"], end="\t")
                st.write("\t", matches["series_dates"])

    elif option == "Series from Archive":
        st.header("Series from Archive")
        cric_series = get_series_from_archive()
        for series in cric_series:
            st.write(series["series_name"], end="\t")
            st.write(series["series_dates"])

    elif option == "Matches by Day":
        st.header("Matches by Day")
        cric_series = get_matches_by_day()
        for day in cric_series:
            st.write(day)
            for match in cric_series[day]:
                st.write("\t", match["match_name"], end="\t")
                st.write("\t", match["start_date_time"])

    elif option == "Series Matches":
        st.header("Series Matches")
        cric_series_matches = get_series_matches()
        for match in cric_series_matches:
            st.write(match["match_title"])

    elif option == "Player Stats":
        st.header("Player Stats")
        player_stats = get_player_stats()
        for player in player_stats:
            st.write(player["Player"], end="\t")
            st.write(player["Runs"])

    elif option == "Teams List":
        st.header("Teams List")
        teams = get_teams_list()
        for team in teams:
            st.write(team["team_name"], end="-")
            st.write(team["team_code"])

    elif option == "Team Schedule":
        st.header("Team Schedule")
        # Replace '2' with the actual team_id you want to display the schedule for
        team_schedule = get_team_schedule(2)

        if team_schedule:
            schedule_data = []
            headings = ["Match Title", "Start Date/Time", "Location", "Series Name"]

            # Append headings to schedule_data
            schedule_data.append(headings)

            for match in team_schedule:
                schedule_data.append([
                    match.get("match_title", ""),
                    match.get("match_start_data_time", ""),
                    match.get("match_location", ""),
                    match.get("match_series_name", "")
                ])

            st.table(schedule_data)
        else:
            st.markdo
    elif option == "Team Results":
        st.header("Team Results")
        # Replace '2' with the actual team_id you want to display the results for
        team_results = get_team_results(2)
        for result in team_results:
            st.write(result["match_status"])
